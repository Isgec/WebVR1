...
Partial Class EF_vrRequestExecution
  ...
  Protected Sub GVvrLinkedRequest_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVvrLinkedRequest.RowCommand
    ...
    If e.CommandName.ToLower = "rejectwf".ToLower Then
      If PODataPushed Then
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", "alert('" & New JavaScriptSerializer().Serialize("PO Data is pushed in ERP. Can NOT be modified.") & "');", True)
      Else
        Try
          Dim RequestNo As Int32 = GVvrLinkedRequest.DataKeys(e.CommandArgument).Values("RequestNo")
  ...
  Protected Sub ODSvrRequestExecution_Selected(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.ObjectDataSourceStatusEventArgs) Handles ODSvrRequestExecution.Selected
    ...
    Dim prjList As List(Of String) = oVR.ProjectList
    Dim isErr As Boolean = False
    Dim erStr As String = ""
    For Each prj As String In prjList
      Dim ePO As SPApi.POData = SPApi.POData.GetByID(oVR.ISGECLoadID, prj, Comp)
    ...
    Next
  End Sub
  Public Property PODataPushed() As Boolean
    Get
      If ViewState("PODataPushed") IsNot Nothing Then
        Return CType(ViewState("PODataPushed"), Boolean)
      End If
      Return 0
    End Get
    Set(ByVal value As Boolean)
      ViewState.Add("PODataPushed", value)
    End Set
  End Property
